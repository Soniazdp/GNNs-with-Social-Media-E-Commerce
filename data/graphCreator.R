...
# #------------------------------
elist <- c()
ht <- read.csv("ht_unsort_makeuptutorial.csv", sep = ",", stringsAsFactors = FALSE, header = T, fileEncoding = "UTF-8")
...
write.csv(elist_m, "edgelist_makeuptutorial.csv", fileEncoding = "UTF-8", row.names=F)

# #---------------
# #Load Edge List
# #---------------
imp_matrix <- as.matrix(read.csv("edgelist_makeuptutorial.csv", sep = ";"))
...
#Subgraph
graph6 <- induced.subgraph(graph2, which(dgr>80))